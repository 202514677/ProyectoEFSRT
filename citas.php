...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['registrar_cita'])) {
    ...
    if ($hora < "08:00:00" || $hora > "18:00:00") {
        ...
        $stmt = $pdo->prepare("INSERT INTO citas (id_paciente, id_medico, fecha_cita, hora_cita, motivo) VALUES (?, ?, ?, ?, ?)");
        if($stmt->execute([$id_paciente, $id_medico, $fecha, $hora, $motivo])) {
...
$sql = "SELECT c.*, CONCAT(p.nombre, ' ', p.apellido) AS paciente, CONCAT(m.nombre, ' ', m.apellido) AS medico FROM citas c JOIN pacientes p ON c.id_paciente = p.id_paciente JOIN medicos m ON c.id_medico = m.id_medico WHERE 1=1";
    ...
    <div class="table-responsive">
        <table>
            ...
            <tbody>
                ...
                <tr><td><?= $c['id_cita']; ?></td><td><?= htmlspecialchars($c['paciente']); ?></td><td><?= htmlspecialchars($c['medico']); ?></td><td><?= $c['fecha_cita']; ?></td><td><?= date('h:i A', strtotime($c['hora_cita'])); ?></td><td><span style="padding:4px 8px; background:#f1c40f; border-radius:4px;"><?= $c['estado']; ?></span></td></tr>
                <?php endforeach; ?>
            </tbody>
        </table>
    </div>
